...
- `priceAt(seed, tick, district, good)` (`src/sim/crime/contraband.ts`) is the only price there is.
  It is a pure function, like the traffic and the crowd of spec section 5.3, so no table of prices
  is kept, stepped or saved.
...
- `GOODS` (`src/sim/crime/goods.ts`) is the list: twelve goods in three groups, each with a name, a
...
- The panel draws the list in its own order, under the headings of `GOOD_GROUPS`. So a group has to
  be one run of the list, and the test in `test/sim/crime/market.test.ts` holds that, cheapest first
  inside a run.
...
- One dealer works each district. `dealerPlaces(seed, districts, snap)` (`src/sim/crime/dealer.ts`)
  picks `PITCHES` corners spread round the district's site and hands each to `snap`. The snap is
  handed in rather than imported so that a simulation test can put a dealer on a hillside with no
  roads on it.
- In the game the snap is `kerbsidePlace` (`src/world/roads/kerbside.ts`), not `nearestRoadPlace`.
  The second answers the middle of the carriageway, which is where a car stands; a dealer put there
  ...
  gives up. `test/sweep/seed-places.test.ts` checks no pitch of the sweep stands in a road.
...
- `stepMarket` (`src/sim/crime/market.ts`) runs after `stepShops` and before the physics. It takes
  the interact key only after the shops have had it, so a press that opened a shop door does not
  also open a deal, and the vehicle still wins the key wherever both could take it.
...
- `src/ui/panels/trade-panel.ts` is built on the shop counter's layout and classes (`shop.css`),
  with the parts a trade adds in `trade.css`. Keep the two alike, so a dealer reads like every other
  counter in the game.
...
- The preview is the shop's `ShopPreview`, with a look of kind `good`. The props are in
  `src/render/crime/contraband-props.ts`, and `--gallery=goods` lays them all out in one picture.
...
- `dollars()` (`src/sim/crime/market.ts`) groups the digits by hand. `toLocaleString` reads the
  machine's own locale, and two machines would then write two different records.

## The map and the crowd

- `DealerMarks` (`src/ui/hud/dealers.ts`) owns both of the dealers' bodies on screen: the `dealer`
  mark on the maps and the person standing on the corner. It rewrites them when the spell turns and
...
- That list is not written here, though. `EnforcerMarks` (`src/ui/hud/enforcers.ts`) writes it for
  the dealers and the faction enforcers of spec section 17.2 together, because the mesh reads one
  list and the enforcers move every tick. `DealerMarks.marks` is what it reads the dealers' own
  marks off, and not `MapPois.extra`, which it is about to replace.
