 ...
 * `src/sim/traffic/parked.ts`. The bays are plain arrays, one entry per bay, so a chunk
...
import { layoutZones, zoneAt } from '../terrain/districts.ts';
import type { RoadCarve } from '../carve/carve.ts';
import type { JunctionMap } from '../junctions/junctions.ts';
import { insideRegion, ringsClash } from './lot-geom.ts';
import type { Parcel, ParcelMap } from './parcels.ts';
import { footprintHalfWidth, TIERS } from '../roads/tiers.ts';
import { AIRCRAFT_CLASSES, type AircraftStand, type RoadCurve, type RoadTier, type WorldDescription, type Zone } from '../types.ts';
import { STREET_REACH } from '../terrain/vegetation.ts';
