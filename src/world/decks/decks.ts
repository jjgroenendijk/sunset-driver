...
import { RoadBeds, surfaceHeight } from '../carve/bed.ts';
import { buildRoadGraph } from '../roads/graph.ts';
import { buildJunctions } from '../junctions/junctions.ts';
...
export function deckSurfaceAt(decks: readonly DeckSpan[], x: number, y: number): number | undefined {
  ...
  for (const span of decks) {
    ...
    for (let i = 0; i + 1 < span.points.length; i++) {
      const height = segmentSurfaceAt(span, span.points[i] as DeckPoint, span.points[i + 1] as DeckPoint, x, y);
...
/** The height of the surface one segment of a deck carries at a place, or undefined where it does not cover it. */
function segmentSurfaceAt(span: DeckSpan, a: DeckPoint, b: DeckPoint, x: number, y: number): number | undefined {
  const dx = b.x - a.x;
  const dy = b.y - a.y;
  ...
  if (Math.abs(off) > span.halfWidth || hypot(x - cx, y - cy) > span.halfWidth) return undefined;
  return a.height + (b.height - a.height) * t + (a.bank + (b.bank - a.bank) * t) * off;
}
