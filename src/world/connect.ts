...
import { crossingsOf } from './connect-search.ts';
...
// The search for the crossings is next door, and `crossPoint` comes out
// through here, as it did while the two were one file.
export { crossPoint } from './connect-search.ts';
...
function plan(
  ...
  // A bend onto a snapped place may not fold the road back over itself.
  if (bendOverlaps(road.points, segment, spot, road.tier)) return undefined;
