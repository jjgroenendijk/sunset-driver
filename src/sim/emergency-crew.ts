 ...
 * reads it and adds nothing: `render/emergency-crew.ts` lays the hose from
 * where a firefighter stands, `ui/emergency-crews.ts` walks their bodies in
 * the crowd's mesh, and `render/casualties.ts` kneels a medic who has reached
 * a body.
 */
import { atan2, cos, hypot, sin } from '../core/libm.ts';
import { casualtyPose, emptyCasualtyPose, type Casualty } from './casualty-motion.ts';
import { TICK_RATE } from './clock.ts';
import { COLLECT_RANGE, HOSE_RANGE, UNIT_BODY, type EmergencyKind, type EmergencyUnit } from './emergency.ts';
import { MAX_HEALTH } from './on-foot.ts';
 ...
 * hands rather than the engine (`render/emergency-crew.ts`).
 */
 ...
 * `render/preview.ts` and a test stand them where a tick of work would have.
 */
