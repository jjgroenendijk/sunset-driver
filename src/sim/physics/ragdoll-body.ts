 ...
 * crowd's rig (`src/render/people/pedestrian-rig.ts`), and the joints between them.
 *
 * The numbers are the rig's, copied here because `src/sim` may not import
 * `src/render`. `test/sim/physics/ragdoll.test.ts` holds the two to the same joints.
 *
 ...
 */
import { STRIDE_HEIGHT } from '../crowd/pedestrian-look.ts';
