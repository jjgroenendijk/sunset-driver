...
import { groundPart } from '../environment/ground.ts';
...
import type { RoadScenery } from '../roads/roads.ts';
import type { SignScenery } from '../signage/signs.ts';
import { detailAt, spendBudget, wantedChunks, type ChunkDetail, type TilePart } from './streaming.ts';
import type { PlantScenery } from '../environment/vegetation.ts';
...
export class ChunkTiles {
   ...
   * takes the sun's shadow, and casts one unless its piece says otherwise:
   * paving and paint on the ground shade only themselves (`roads.ts`). `mirrored` is the same answer for the water's mirror, which
