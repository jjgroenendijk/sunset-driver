 ...
 * cover. This file is the door onto both. The ink lines around a building come
 * from the screen-space edge pass of `edges.ts`, not from geometry.
 *
...
import { lotMiddle, type Building, type BuildingKind } from '../../world/city/buildings.ts';
...
import type { ChunkDetail } from '../streaming/streaming.ts';
...
export interface BuildingPlacement {
   ...
   * the shell: the camera reads the box of the shell (`roofs.ts`), so a mast
   * here is never something the camera climbs.
   */
 ...
 * is the massing alone, one box per building.
 */
...
/** Vertices a chunk's buildings cost: every shell, every roof and every footing. */
export function buildingVertices(placements: readonly BuildingPlacement[]): number {
  let count = 0;
  for (const placed of placements) {
    count += placed.shell.getAttribute('position').count;
 ...
 * and one of blocks. A chunk with no building of a kind pays nothing for it.
 */
