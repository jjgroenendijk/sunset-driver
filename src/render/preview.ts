...
async function draw(request: PreviewRequest): Promise<PreviewResult> {
  ...
  const { traffic, trams, busStops, crowd, casualties, guns, markers, wildlife, parked } = peopleFor();
  ...
  const ambient = [traffic.group, trams.group, busStops.group, crowd.group, casualties.group, wildlife.group, parked?.group];
