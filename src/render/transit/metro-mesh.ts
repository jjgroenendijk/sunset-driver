 ...
 * is the way in. `src/world/transit/metro.ts` says where that is — the middle of the
...
import type { MetroEntrance } from '../../world/transit/metro.ts';
import type { SurfaceAt } from '../roads/pavement-mesh.ts';
import { vergeRise } from '../roads/road-section.ts';
...
export interface MetroStair {
  ...
  y: number;
  height: number;
  /** Radians, from the stairs toward the road (`src/world/transit/metro.ts`). */
  heading: number;
}
