 ...
 * 11.5).
 *
 * `src/sim/boarding.ts` says that a move is running, which way, through which
...
import { ridePose } from './rider.ts';
import { doorOf, saddleOf, seatOf, type Door } from './vehicle-mesh.ts';
import { DOOR_STAND, doorAlong, type BoardingWay } from '../sim/boarding.ts';
import { EXIT_CLEARANCE, WALK_SPEED } from '../sim/on-foot.ts';
 ...
 * `exitPlace` in `src/sim/on-foot.ts` stands the player: beside the middle of
