...
- `src/core/geom.ts` is the polygon arithmetic the parcel model runs on: `strip`, `disc`, `union`, `difference`, `ringArea`, `pointInRing`. A `Region` is an outer ring wound anticlockwise with its holes wound clockwise. `union` and `difference` round every coordinate to the millimetre, so the cross products that say which side of an edge a point falls on are exact integers and can never contradict one another. Over a whole network they cost about a third of a world generation, so a caller keeps the answer instead of asking twice.
