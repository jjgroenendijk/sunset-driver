/**
 * Measure what the audio of spec section 15 actually makes, since a headless
 * run has no ears.
 *
 ...
 * offline audio context. Each line is one case: its peak, its loudness and how
 * much of it was silence.
 *
 * It fails when a case that should make a sound is silent, or when one clips,
 * which is what a broken voice, a node that was never connected and a mix that
 * is too hot all look like from here.
 *
...
try {
  ...
  for (const level of levels) {
    ...
    const clipping = level.peak > 1;
    const wanted = level.name === SILENT_CASE ? silent : !silent && !clipping;
