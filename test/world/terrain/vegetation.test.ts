...
import type { Building, BuildingMap } from '../../../src/world/city/buildings.ts';
import type { Parcel, ParcelMap, ParcelOwner } from '../../../src/world/city/parcels.ts';
...
import {
  ...
  Vegetation,
  type Plant,
} from '../../../src/world/terrain/vegetation.ts';
...
function parcelOf(owner: ParcelOwner, zone: Zone, ring: Point[] = square(0, 0, SIDE)): ParcelMap {
  ...
  return { parcels: [parcel], area: parcel.area, land: parcel.area, stations: [], metro: [] };
}
...
describe('vegetation', () => {
  it('plants nothing on ground no parcel owns', () => {
    ...
    expect(new Vegetation(SEED, { parcels: [], area: 0, land: 0, stations: [], metro: [] }, NOTHING).plantsIn(WHOLE, [])).toEqual([]);
  });
