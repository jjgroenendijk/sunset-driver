...
describe('the crowd, drawn (spec section 13.1)', () => {
  it('draws every person in view as one instance of one mesh, where their loop puts them', () => {
    ...
    // The first instance stands where the first person in view stands.
    ...
    expect([place.getX(0), place.getY(0), place.getZ(0), place.getW(0)]).toEqual(
      [first.x, first.height, first.y, first.heading].map((v) => Math.fround(v)),
    );
